...
#if defined(NDEBUG)
# error "warpcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd2a9feff5e0ac17fffceef6ccab56a755f111b76705d18baea9329ac0e98fb1b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // warpcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "warpcoin Signed Message:\n";
...
{
    // warpcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // warpcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // warpcoin: 1 days
static const int64 nTargetSpacing = 0.5 * 10; // warpcoin: 5 Seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // warpcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // warpcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe5dc2bcd82082b8f08f3130bd3054d783bb39fbd1a52b66fef80400dd75e524d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x4736e074c0ab1c9bb338594dbfa88d8e27b206240e96429259508f99ae325b2b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // warpcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("warpcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("warpcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static warpcoinMiner(CWallet *pwallet)
{
    printf("warpcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("warpcoin-miner");
    ...
    try { loop {
        ...
        printf("Running warpcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("warpcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&warpcoinMiner, pwallet));
}
